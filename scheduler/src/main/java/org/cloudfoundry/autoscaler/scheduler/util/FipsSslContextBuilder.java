...
public class FipsSslContextBuilder {
  ...
  public static SSLContext buildWithSystemTrust(
    ...
    return buildWithSystemAndCustomTrust(keyManagers, null, protocol);
  }
  ...
  private static X509TrustManager getSystemTrustManager()
    ...
    X509TrustManager x509tm = extractX509TrustManager(tmf);
    logger.info(
  ...
  private static int loadCfTrustedCertificates(KeyStore keyStore) {
    ...
    try {
      String pemContent = Files.readString(cfCertsPath);
      Collection<X509Certificate> certs = new ArrayList<>(FipsPemUtils.parseCertificateChain(pemContent));
  ...
  /**
   * Extracts the first X509TrustManager from a TrustManagerFactory.
   *
   * @return the X509TrustManager
   * @throws IllegalStateException if no X509TrustManager is found
   */
  public static X509TrustManager extractX509TrustManager(TrustManagerFactory tmf) {
  ...
  private static class MergedTrustManager implements X509TrustManager {
    ...
    public MergedTrustManager(
      ...
      // Cache merged issuers — called on every TLS handshake
