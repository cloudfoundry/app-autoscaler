...
class FipsSecurityProviderConfigTest {
  ...
  @Test
  void isFipsModeEnabledReturnsFalseWhenVcapServicesNotSet() {
    // VCAP_SERVICES is not set in test environment, so should return false
    assertThat(FipsSecurityProviderConfig.isFipsModeEnabled()).isFalse();
  }

  @Test
  void isInitializedReturnsTrueAfterInitialization() {
    assertThat(FipsSecurityProviderConfig.isInitialized()).isTrue();
  }
}
