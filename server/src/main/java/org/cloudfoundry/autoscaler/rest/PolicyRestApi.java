   ...
   public Response getPolicy(@Context final HttpServletRequest httpServletRequest, @PathParam("id") String policyId)
	{
		...
		try {
			...
			while  (iter.hasNext()) {  
				...
				else if (scheduledPolicyItemJson.get("type").equals(ScheduledPolicy.ScheduledType.SPECIALDATE.name())) {
					scheduledPolicyItemJson.put("minInstCount", scheduledPolicyItemJson.get("instanceMinCount"));
					scheduledPolicyItemJson.put("maxInstCount", scheduledPolicyItemJson.get("instanceMaxCount"));
			...
			return RestApiResponseHandler.getResponseError(MESSAGE_KEY.RestResponseErrorMsg_build_JSON_error, e, httpServletRequest.getLocale());
	...
	public Response getPolicies(@Context final HttpServletRequest httpServletRequest, @QueryParam("service_id") String serviceId) {
		if (serviceId == null || serviceId.trim().length() == 0) {
			return Response.serverError().entity("Service ID is not specified")
					.build();
		}
			...
			if (applications == null) {
				...
				for (Application app : applications) {
					...
					if ((policyId != null) && (policyId.trim().length() > 0)) {
						...
						try {
							PolicyManager policyManager = PolicyManagerImpl.getInstance();
							AutoScalerPolicy policy = policyManager
							...
							List<AutoScalerPolicyTrigger> triggers = policy
									.getPolicyTriggers();
	...
	private void generatePolicyFromJsonString(AutoScalerPolicy policy, String jsonString) throws IOException {
		...
		if(null != recurringSchedule){
			...
			while(it.hasNext()){
				...
				scheduledPolicy.setInstanceMinCount(Integer.parseInt(String.valueOf(recurringScheduleItemJson.get("minInstCount"))));
				scheduledPolicy.setInstanceMaxCount(Integer.parseInt(String.valueOf(recurringScheduleItemJson.get("maxInstCount"))));
		...
		if(null != specificDate){
			...
			while(it.hasNext()){
				...
				scheduledPolicy.setInstanceMinCount(Integer.parseInt(String.valueOf(specificDateItemJson.get("minInstCount"))));
				scheduledPolicy.setInstanceMaxCount(Integer.parseInt(String.valueOf(specificDateItemJson.get("maxInstCount"))));
				scheduledPolicy.setStartTime(String.valueOf(specificDateItemJson.get("startDate")) + " " + String.valueOf(specificDateItemJson.get("startTime")));
				scheduledPolicy.setEndTime(String.valueOf(specificDateItemJson.get("endDate")) + " " + String.valueOf(specificDateItemJson.get("endTime")));
				scheduledPolicy.setTimezone(timezone);
