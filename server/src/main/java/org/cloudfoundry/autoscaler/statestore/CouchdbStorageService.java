...
import org.cloudfoundry.autoscaler.cloudservice.couchdb.connection.ServerMetricDBConnectionManager;
import org.cloudfoundry.autoscaler.cloudservice.couchdb.connection.ServerSeperatedViewRepoConnectionManager;
...
public class CouchdbStorageService implements AutoScalingDataStore {
    ...
    private TriggerRecordRepositoryCollection triggerRepo;
    private BoundAppRepositoryCollection bindingRepo;
    private ConfigRepositoryCollection configRepo;
    private MetricDBSegmentRepositoryCollection metricDBSegmentRepo;
    private ScalingPolicyRepositoryCollection policyRepo;
    private ApplicationRepositoryCollection appRepo;
    private ScalingStateRepositoryCollection stateRepo;
    private ScalingHistoryRepositoryCollection historyRepo;
    
    
    private List<AppInstanceMetricsRepositoryCollection> AppInstanceMetricsRepoList;
    ...
    private void initCouchdb() {
		...
		ServerSeperatedViewRepoConnectionManager ScalingRepoManager = null;

		ScalingRepoManager = new ServerSeperatedViewRepoConnectionManager(DB_NAME, username, password, host, port,
				enableSSL, timeout, initDesignDocument);
		triggerRepo = ScalingRepoManager.getTriggerRecordRepo();
		bindingRepo = ScalingRepoManager.getBoundAppRepo();
		configRepo = ScalingRepoManager.getConfigRepo();
		metricDBSegmentRepo = ScalingRepoManager.getMetricDBSegmentRepo();
		policyRepo = ScalingRepoManager.getScalingPolicyRepo();
		appRepo = ScalingRepoManager.getApplicationRepo();
		historyRepo = ScalingRepoManager.getScalingHistoryRepo();
		stateRepo = ScalingRepoManager.getScalingStateRepo();
    ...
    public void initExistingMetricDB()  {
        ...
        AppInstanceMetricsRepoList = new ArrayList<AppInstanceMetricsRepositoryCollection>();
        metricDBSegmentList = metricDBSegmentRepo.findLastestMetricDBs(serverName);
		...
		if (metricDBSegmentList != null) {
			...
			// stale the old db segment records & rollout old database
			 ...
			 while(iter.hasNext()){   
			     ...
			     if( seg.getEndTimestamp() != Long.MAX_VALUE && seg.getEndTimestamp() + metricDBStaleTime < curSegment.getStartTimestamp()){   
			    	...
			    	logger.info("Remove staled metricDB : " + metricDBPrefix + seg.toString());
			 		new ServerMetricDBConnectionManager(staledMetricDBName, username, password, host, port, enableSSL, timeout)
			 			.deleteMetricDB(staledMetricDBName);
			    	 metricDBSegmentRepo.remove(seg);
			    	 iter.remove(); 
			     }   
			 }   			
		}else
    	...
    	if ( metricDBSegmentList == null || metricDBSegmentList.size() == 0 ) {
    		...
    		if (! (curSegment.getMetricDBPostfix().equalsIgnoreCase(lastestPreSegment.getMetricDBPostfix()) 
    			...
    			//cut off preSegment endTimestamp record to reflect the actual end time. 
        		if (lastestPreSegment.getEndTimestamp() > curSegment.getStartTimestamp()) {
        			...
        			metricDBSegmentRepo.updateMetricDBSegment(lastestPreSegment);
        		}
    		}
    	}
    	...
    	if (createNewSegment) {
    		...
    		metricDBSegmentRepo.add(curSegment);
    	}

    	for (MetricDBSegment seg : metricDBSegmentList){
    		String metricDBName = metricDBPrefix + seg.getMetricDBPostfix();
    		ServerMetricDBConnectionManager manager = null;
    		
    		manager = new ServerMetricDBConnectionManager(metricDBName,username, password, host, port, enableSSL, timeout, true);
    		
    		AppInstanceMetricsRepositoryCollection appInstanceMetricsRepo = (manager).getAppInstanceMetricRepo();
    		AppInstanceMetricsRepoList.add(appInstanceMetricsRepo);
    	}
    ...
    public synchronized boolean addMetricDB(long startTimestamp, int seq) throws Exception {
    	...
    	if (metricDBSegmentRepo.findByServerNameSegmentSeq(serverName, seq) != null ){
    	...
    	metricDBSegmentRepo.add(newSegment);

   		String newMetricDBName = metricDBPrefix + newSegment.getMetricDBPostfix(); 
    	AppInstanceMetricsRepositoryCollection newAppInstanceMetricsRepo = null;
    	try { 
    		newAppInstanceMetricsRepo = (new ServerMetricDBConnectionManager(newMetricDBName,username, password, host, port, enableSSL, timeout, true)).getAppInstanceMetricRepo();
		...
		AppInstanceMetricsRepoList.add(newAppInstanceMetricsRepo);
    	metricDBSegmentList.add(newSegment);
    	curMetricDBSegmentAnchor = metricDBSegmentList.size()-1; 
    	logger.info("Add a new metric DB. Current metricDBSegment is : " + mapper.writeValueAsString(metricDBSegmentList));
    ...
    public void addTrigger(Trigger t) throws Exception {
        ...
        TriggerRecord existingRecord = null;
            existingRecord = triggerRepo.get(triggerRecord.getId());
            if (existingRecord != null) {
                triggerRepo.remove(existingRecord);
            }

        triggerRecord.setServerName(AutoScalerEnv.getServerName());
        triggerRepo.add(triggerRecord);
    }

    @Override
    public void removeTrigger(String appId) throws Exception {
        triggerRepo.removeByAppId(appId);

    }

    @Override
    public Map<String, List<TriggerRecord>> getAllTriggers() throws Exception {
        return triggerRepo.getAllTriggers(serverName);
    }
    ...
    public void addAppStats(AppInstanceMetrics appInstanceMetrics) throws Exception {
		...
		AppInstanceMetricsRepositoryCollection activeAppInstanceMetricsRepo = AppInstanceMetricsRepoList.get(curMetricDBSegmentAnchor );
    	
    	long currentTimestamp = appInstanceMetrics.getTimestamp();
    	long endTimestamp = activeMetricDBSegment.getEndTimestamp();
    	
    	if (currentTimestamp > endTimestamp){
    		// if a new metric db is added and connected, add to the new metric db, 
    		//  otherwise reusing previous ones and retry the add metric db when next metric data comes.
    		if ( addMetricDB(currentTimestamp, activeMetricDBSegment.getSegmentSeq() + 1) )
   				activeAppInstanceMetricsRepo = AppInstanceMetricsRepoList.get(curMetricDBSegmentAnchor);
    	}
    		
		activeAppInstanceMetricsRepo.add(appInstanceMetrics);
    }
    ...
    public List<AppInstanceMetrics> getAppStatsHistoryByAppIdAfter(String appId, long newerThan) throws Exception {
		...
		for (int i = startingAnchor; i <= curMetricDBSegmentAnchor; i++) {
			AppInstanceMetricsRepositoryCollection appInstanceMetricsRepo = AppInstanceMetricsRepoList.get(i);
			List<AppInstanceMetrics> appInstanceMetrics = appInstanceMetricsRepo.findByAppIdAfter(appId,newerThan); 
    ...
    public ServiceConfig getConfig(String serviceId) throws Exception {
        ...
        config = configRepo.get(serviceId);
        return config;
    }
    ...
    public Map<String, List<BoundApp>> getAllBindings() throws Exception {
        ...
        List<BoundApp> boundApps = bindingRepo.getAllBoundApps(serverName); // will add this line later
    ...
    public List<BoundApp> getAllBindingsByServiceId(String serviceId) throws Exception {
        return bindingRepo.findByServiceId(serviceId);
    }
    ...
    public String getAppTypeById(String appId) throws Exception {
        BoundApp boundApp = bindingRepo.findByAppId(appId);
        if (boundApp != null) {
    ...
    public Application getApplicationByBindingId(String bindingId) {
        return appRepo.findByBindingId(bindingId);
    }
    ...
    public AppAutoScaleState getScalingState(String appId) {
        return stateRepo.findByAppId(appId);
    }

    @Override
    public List<Application> getApplications(String serviceId) {
        return appRepo.findByServiceIdAndState(serviceId);
    }
    ...
    public void saveScalingHistory(ScalingHistory scalingHistory) throws DataStoreException {
        try {
			...
			if (existingHistory == null)
				historyRepo.add(scalingHistory);
			else {
				...
				historyRepo.update(scalingHistory);
			}
  		} catch (org.ektorp.DbAccessException e) {
  			throw new DataStoreException(e);
  		}
    ...
    public List<ScalingHistory> getHistoryList(ScalingHistoryFilter filter) throws DataStoreException {
        List<ScalingHistory> historyList = historyRepo.findByScalingTime(filter.getAppId(), filter.getStartTime(),
                filter.getEndTime());
	...
	public List<Application> getApplicationsByPolicyId(String policyId) {
		return appRepo.findByPolicyId(policyId);
	}
	...
	public List<AutoScalerPolicy> getAutoScalerPolicies() {
		return policyRepo.getAllRecords();
	}
