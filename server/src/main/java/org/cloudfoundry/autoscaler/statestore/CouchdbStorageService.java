...
import org.cloudfoundry.autoscaler.cloudservice.couchdb.connection.ServerViewDAOManager;
...
public class CouchdbStorageService implements AutoScalingDataStore {
	...
	private void initCouchdb() {
		...
		ServerViewDAOManager ScalingRepoManager = null;

		ScalingRepoManager = new ServerViewDAOManager(DB_NAME, username, password, host, port,
	...
	public void initExistingMetricDB() {
		...
		if (metricDBSegmentList != null) {
			...
			while (iter.hasNext()) {
				...
				if (seg.getEndTimestamp() != Long.MAX_VALUE
					...
					new ServerMetricDBConnectionManager(staledMetricDBName, username, password, host, port, enableSSL,
		...
		for (MetricDBSegment seg : metricDBSegmentList) {
			...
			manager = new ServerMetricDBConnectionManager(metricDBName, username, password, host, port, enableSSL,
					timeout, true);
	...
	public synchronized boolean addMetricDB(long startTimestamp, int seq) throws Exception {
		...
		try {
			newAppInstanceMetricsDao = (new ServerMetricDBConnectionManager(newMetricDBName, username, password, host,
