package org.cloudfoundry.autoscaler.metric.config;
...
import org.cloudfoundry.autoscaler.metric.StateMonitor;
...
//    private static final int DEFAULT_TIMEOUT = 120;
	...
	private String[] metricPriorityArray = ConfigManager.get(Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR  + 
				Constants.METRIC_SOURCE + Constants.METRIC_SEPERATOR  + "priority", 
				"poller").replaceAll("\\s*", "").split(",");  
    ...
    public Map<String, Object> loadDefaultConfig(String appType, String appId) {
		...
		HashMap<String, Object> customConfig = (HashMap<String, Object>) ((HashMap<String, Object>)defaultConfig).clone();
		for (Entry<String, Object> entry : customConfig.entrySet()){
			Object value = entry.getValue();
			if ( value instanceof HashMap<?,?>){
				HashMap<String, Set<String>> clonedValue = (HashMap<String, Set<String>>) ((HashMap<?,?>)value).clone();
		...
		HashMap<String, Set<String>> metricMap = (HashMap<String, Set<String>>) customConfig.get(Constants.METRICS_CONFIG);
		for (Entry<String, Set<String>> entry : metricMap.entrySet()){
			...
			if (isCPURequired){
				if (enabledMetric != null){
					...
					try {
						logger.debug(new StringBuilder().append("Wrong config item for app ").append(appId).append( mapper.writeValueAsString(customConfig)));
					} catch (Exception e) {
		...
		try {
			logger.debug(new StringBuilder().append("New config item for app ").append(appId).append( mapper.writeValueAsString(customConfig)));
		} catch (Exception e) {
			logger.info(e.getMessage());
		}
    ...
    public Map<String, Object> loadDefaultConfig(String appType) {
    	...
    	if ( defaultConfigMap.get(key) == null) {
    		...
    		String metricSource=ConfigManager.get( Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR + appType, 
    								ConfigManager.get(Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR + Constants.METRIC_WILDCARD));	
    		...
    		for (String source : metricSourceArray){
    			...
    			boolean enabled=ConfigManager.getBoolean(Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR  + 
						appType + Constants.METRIC_SEPERATOR + 
						source +  Constants.METRIC_SEPERATOR +
						Constants.METRIC_SOURCE_STATUS ,  
							ConfigManager.getBoolean(Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR  + 
								Constants.METRIC_WILDCARD + Constants.METRIC_SEPERATOR + 
								source +  Constants.METRIC_SEPERATOR +
								Constants.METRIC_SOURCE_STATUS, true));
				...
				String enabledMetricString = ConfigManager.get(Constants.METRIC_PREFIX + Constants.METRIC_SEPERATOR  + 
															appType + Constants.METRIC_SEPERATOR + 
															source +  Constants.METRIC_SEPERATOR +
															Constants.METRIC_SOURCE_TYPE,  
	...
	public Set<String> getEnabledMetric (String appType, String dataSource, String appId){
    	...
    	if (dataSource == null || appType== null )
     	...
     	if (appId != null) {
   		...
   		String key = isCPURequired || isPollerMemRequired ? new StringBuilder().append(appType).append("#").append(appId).toString() : appType;
   		...
   		try {
			logger.debug(new StringBuilder().append("Enabled metric for ").append(appId).append(" with datasource ").append(dataSource).append(" : ").append(mapper.writeValueAsString(enabledMetric)));
		} catch (Exception e) {
			logger.info(e.getMessage());
		}
