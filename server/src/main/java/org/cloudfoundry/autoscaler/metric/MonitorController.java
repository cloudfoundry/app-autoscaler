...
/**
 * @author shicong, paolo, peterw
 * 
 *         peterw: this is a rename and rewrite of the original RunTime class in the monitor. This class is the central
 *         controller that manages all monitoring events and actions.
 */
public class MonitorController implements Runnable {
    ...
    private MonitorController() {
        ...
        purgeLocalCacheExecutor.scheduleWithFixedDelay(new PurgeLocalCacheThread(),
    ...
    public void addTrigger(Trigger t) throws Exception {
        ...
        try {
            ...
            // check if the corresponding state monitor exists, if not, create one
    ...
    public void addPoller(String appId) {
    	...
    	if (ConfigService.getInstance().getEnabledMetric(getAppType(appId), Constants.METRIC_SOURCE_POLLER, appId) == null){
    ...
    public void processAppInstanceMetrics(AppInstanceMetrics appInstanceMetrics, String dataSource /*poller */){
    	try {
    		...
    		//push to evaluation. 
    		AppInstanceMetrics clonedAppInstanceMetrics = filterAppInstanceMetricsForEvaluation(appInstanceMetrics, dataSource);
    ...
    private AppInstanceMetrics filterAppInstanceMetricsForEvaluation(AppInstanceMetrics appInstanceMetrics, String dataSource) {
        ...
        //Now, the clonedAppInstanceMetrics need to be evaluated. Clone it as a new data, and get rid of the non-interested data.
    ...
    public void addAppInstanceMetrics(AppInstanceMetrics appInstanceMetrics, String dataSource) {
        ...
        if (appMetrics == null) {// no active record for this app, 
        	logger.debug(new StringBuilder().append("The target app ").append(appId).append(" is not initialized. Add to appMetrics map now." ));
        ...
        if (instanceMetricsList != null) {
        	 if (dataSource.equalsIgnoreCase(Constants.METRIC_SOURCE_POLLER)) {
            	//for poller metrics, push to appMetricsMap first, then store to db
    ...
    private void storeAppInstanceMetrics(String appId) {
    	...
    	try {
			...
			AppInstanceMetrics storedAppInstanceMetrics = appMetricsMap.get(appId).mergeToAppInstanceMetrics(true, false);
			AutoScalingDataStoreFactory
			.getAutoScalingDataStore().addAppStats(storedAppInstanceMetrics);
    		...
    		for (Metric metric : instanceMetrics.getMetrics()) {
    			...
    			if (testMetrics.containsKey(key)){
       			...
       			if (count == testMetricSize) 
       				break; //break the metric loop since all matches are replaced with the test value
        	}
 		}
   	}	
    ...
    class EvaluationTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // get StateMonitor for this app, and only if it exists we update its data
                    ...
                    if (triggerEventList != null && !triggerEventList.isEmpty()) {
                        for (MonitorTriggerEvent event: triggerEventList){
                            if (ScalingEventManager.getInstance().addTriggerEvents(event)){
                            	...
                            	loggerEvent.debug("Ignore event " + event.toString() + " as the same event type is in queue" );
                            }
                    	}
                    }
                }
    ...
    public void updateAppNameAndType(String appId, String appName, String appType) throws Exception {
        ...
        if (boundApp != null) {
            ...
            AutoScalingDataStoreFactory
			.getAutoScalingDataStore().updateBinding(boundApp.getServiceId(), appId, appType, boundApp.getAppName());
        }
    }
    ...
    public void unbindService(String serviceId, String appId) throws Exception {
    	AutoScalingDataStoreFactory
		.getAutoScalingDataStore().removeBinding(serviceId, appId);
        if (ConfigManager.getBoolean("removeAppHistoryWithUnbind", false))
        	AutoScalingDataStoreFactory
			.getAutoScalingDataStore().removeAppStatsWithHistory(appId);
        removeAppfromSerivcieAppsMap(serviceId, appId);
    }

    public void bindService(String serviceId, String appId)
            throws Exception {
        String appType = null;
        String appName = null;
        try {
        ...
        AutoScalingDataStoreFactory
		.getAutoScalingDataStore().addBinding(serviceId, appId, appType, appName);
    }
    		...
    		if(null != appList){
        		for(BoundApp app : appList){
        			...
        			if(app.getAppName() != null && !app.getAppName().equals(appName)){
        				...
        				AutoScalingDataStoreFactory
        				.getAutoScalingDataStore().addBinding(app.getServiceId(), appId, app.getAppType(), appName);
        			}
        		}
        	}
