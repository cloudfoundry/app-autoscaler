...
/**
 * A state monitor is created for each application
...
public class StateMonitor {
	...
	public static String autoScalerURL = (System.getenv("autoscalerURL")!=null) ? System.getenv("autoscalerURL") : "http://localhost:9080/autoscaler";
		...
		for (InstanceMetrics instanceMetrics : instanceMetricsList){
			...
			for (Metric metric : metrics){
				...
				if (compoundName.equalsIgnoreCase(Constants.METRIC_POLLER_CPU)){
					this.addTuple(new Tuple(Trigger.METRIC_CPU, Double.valueOf(metric.getValue())*100, metric.getTimestamp(), instanceIndex));
					continue;
				}
				...
				if (compoundName.contains("memory")){
					...
					if ( (usedMemory>0) && (maxMemory>0) ) {
						this.addTuple(new Tuple(Trigger.METRIC_MEM, usedMemory, metric.getTimestamp(), instanceIndex, maxMemory));
					}
					continue;
				}
	...
	public void addTuple(Tuple tp)
	{
		synchronized (this.instanceMetricTupleMap) {
			...
			logger.debug("Adding a tuple ["+tp.toString()+"] to state monitor. Key is " + key + ". AppId is " + appId);
	...
	public synchronized void addTrigger(Trigger t) 
	{
		...
		// if the trigger already exists we don't do anything (no need to throw an exception)
	...
	{
		String key = t.generateKey();
		// if the trigger does not exists we don't do anything (no need to throw an exception)
		if ( ! triggerMap.containsKey(key)) {
			logger.warn("Attempting to remove a non-existing trigger, ID = "+t.getTriggerId());
			return;
		}
		
		// remove the trigger from the list of triggers and the breach counter list
		...
		synchronized (this.triggerMap) {
			synchronized (this.instanceMetricTupleMap) {
				for (Trigger t : this.triggerMap.values()) {					
					...
					double quota = 0;					
					AggregationType statType = AggregationType.valueOf(t.getStatType().toUpperCase());//stat type, MAX or AVG					
					Statistic globalStat = new Statistic(statType); // globalStat is used calculate and store statistic value of this app
					
					if (this.instanceSet.isEmpty()) {
						logger.warn( "No monitoring data avaiable for trigger [" + t.getMetric() + ":" + 
								t.getTriggerId() + "] with appId " + appId + ". Possile wrong appId specified in the trigger");
					}
					...
					for (String instanceId : this.instanceSet) {
						...
						if (null == tl || tl.size() == 0) {
                            logger.warn( "No tuples found for app: " + appId + " for trigger: key="  + key + " , thresholdType=" + t.getThresholdType());
                            continue;
						}

						logger.debug("Found ["+tl.size()+"] tuples for app " + appId + " for trigger: key=" + key + " , thresholdType=" + t.getThresholdType());
						...
						synchronized (tl) {
							...
							while (iterator.hasNext()) {
								...
								if (isValidTuple(tp, statWindow, curTime, maxTimeToKeepInSec )){
									...
									/** calculate the statistic value of this instance **/
									instanceStat.update(value);
									logger.debug("update instanceStat for app " + appId + " for instance : " +  instanceId + "  with value " + instanceStat.getValue());
								}else
									iterator.remove();
							}
						}
						...
						if (instanceStat.getCount() > 0) {
							logger.debug("[" + instanceStat.getCount()+"] valid tuples for key ["+key+"] :" + t.getThresholdType() + " for app " + appId + " instanceId " + instanceId);
							/** Calculate the statistic value of the metric **/
							globalStat.update(instanceStat.getValue());
							logger.debug("update glabalStat for app " + appId + " with value " + globalStat.getValue() + " with count " + globalStat.getCount());
							// avoid marking an instance as inactive just because it fails to report some (not all) metric values
							inactiveInstanceList.remove(instanceId);
						} else {
							logger.debug("No valid tuple could be used for key ["+key+"], adding instanceId ["+instanceId+"] to inactive list");
							// the instance is not sending any data within the statWindow, remove the instance
							inactiveInstanceList.add(instanceId);
						}
						
						logger.debug("Aggreated ["+t.getMetric()+"] for instance ["+ 
								instanceId +"] in app ["+this.appId+"]: " + instanceStat.getValue());
					}
					
					if (globalStat.getCount() == 0) 
						continue;
					
					//If the all the metric statistic values reaches the threshold during the breach duration, fire a event
					if (shouldFireEvent (t, quota, globalStat)) {
						MonitorTriggerEvent event = createEvent(t, globalStat);
						eventList.add(event);
						logger.debug("Create an event " + event.toString() + " for app " + t.getAppId() + ". Threshold type is " + t.getTriggerId());
					}
				}
			}
		}
	...
	private boolean isValidTuple (Tuple tp, int statWindow, long curTime, long maxTimeToKeepInSec){
		...
		logger.debug("Checking tuple ["+tp.toString()+"], " +
				"current timestamp ["+curTime+"], " + 
						"time difference ["+(curTime - tp.timestamp)/1000+"] s");
		/** remove expired tuples **/
		if ((curTime - maxTimeToKeepInSec * 1000) > tp.getTimestamp()) {
			// expired tuple
			logger.debug("Removing an expired tuple ["
					+tp.toString()+"], current timestamp ["+curTime+"]");
			return false;
		/** check if the tuple is in the stat window **/
			...
			if (tp.getEvaluateCount() < 2 ) // the min value of evaluation count should be 2 
				logger.warn("Delete un-evaluated tuple ["
					+tp.toString()+"] of application " + appId + " with current timestamp ["+curTime+"] while the evaluation count is " + tp.getEvaluateCount());
			else 
				logger.debug("Expired tuple ["
					+tp.toString()+"]  with current timestamp ["+curTime+"] while the evaluation count is " + tp.getEvaluateCount());
			return false;
		}
		return true;
	}
	...
	private boolean checkBreachDuration (double threshold, Trigger t){
		...
		logger.debug("Current breach duration is " + breachDuration/1000 + 
				" seconds. Expected breach duration is " + t.getBreachDurationSecs());
