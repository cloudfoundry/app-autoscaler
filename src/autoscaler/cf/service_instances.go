...
func (c *cfClient) GetServiceInstancesInOrg(orgGUID, brokerServicePlanGuid string) (int, error) {
	...
	token, err := c.GetTokens()
	if err != nil {
		c.logger.Error("get-service-instances-get-tokens", err)
		return 0, fmt.Errorf("cf-client-get-service-instances-in-org: failed to get token for request to CF API: %w", err)
	}
	req.Header.Set("Authorization", TokenTypeBearer+" "+token.AccessToken)
...
func (c *cfClient) getCCServicePlanGuid(brokerPlanGuid string) (string, error) {
	...
	if err != nil {
		c.logger.Error("get-service-instances-get-tokens", err)
		return "", fmt.Errorf("cf-client-get-service-plan-guid: failed to get token for request to CF API: %w", err)
	}
	req.Header.Set("Authorization", TokenTypeBearer+" "+token.AccessToken)
...
func (c *cfClient) getBrokerPlanGuid(ccServicePlanGuid string) (string, error) {
	...
	if err != nil {
		logger.Error("new-request", err)
		return "", fmt.Errorf("cf-client-get-broker-plan-guid: failed to get token for request to CF API: %w", err)
	}
...
func (c *cfClient) GetServicePlan(serviceInstanceGuid string) (string, error) {
	...
	tokens, err := c.GetTokens()
	if err != nil {
		logger.Error("get-tokens", err)
		return "", fmt.Errorf("cf-client-get-service-plan: failed to get token for request to CF API: %w", err)
	}
	req.Header.Set("Authorization", TokenTypeBearer+" "+tokens.AccessToken)
