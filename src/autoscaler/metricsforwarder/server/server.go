...
func NewServer(logger lager.Logger, conf *config.Config, policyDB db.PolicyDB, sbssDB db.SbssDB, credentialCache cache.Cache, allowedMetricCache cache.Cache, httpStatusCollector healthendpoint.HTTPStatusCollector, rateLimiter ratelimiter.Limiter) (ifrit.Runner, error) {
	...
	mh := NewCustomMetricsHandler(logger, metricForwarder, policyDB, sbssDB, credentialCache, allowedMetricCache, conf.CacheTTL)
