...
var _ = Describe("PublicApiHandler", func() {
	...
	Describe("GetCustomMetricsCredential", func() {
		JustBeforeEach(func() {
			handler.GetCustomMetricsCredential(resp, req, pathVariables)
		})
		BeforeEach(func() {
			pathVariables["appId"] = TEST_APP_ID
		...
		Context("When there is no record in policydb", func() {
			BeforeEach(func() {
				policydb.GetCustomMetricsCredsReturns(nil, sql.ErrNoRows)
			})
			It("should fail with 404", func() {
				Expect(resp.Code).To(Equal(http.StatusNotFound))
				Expect(resp.Body.String()).To(Equal(`{"code":"Not-Found","message":"Custom metric credential does not exist"}`))
			})
		})
		Context("When there is error retrieving data from policydb", func() {
			BeforeEach(func() {
				policydb.GetCustomMetricsCredsReturns(nil, fmt.Errorf("sql db error"))
			})
			It("should fail with 404", func() {
				Expect(resp.Code).To(Equal(http.StatusInternalServerError))
				Expect(resp.Body.String()).To(Equal(`{"code":"Interal-Server-Error","message":"Error retrieving custom metric credential"}`))
			})
		})
		Context("When successfully retrieve data from policydb", func() {
			BeforeEach(func() {
