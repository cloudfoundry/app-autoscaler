...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if grantType != "password" && grantType != "client_credentials" {
		...
		w.Write(cf.CreateJsonErrorResponse("Error-login-CF", "invalid grant_type"))
		return
	}
	...
	if grantType == "password" {
		if authHeader != "Basic Y2Y6" {
			...
			w.Header().Set("Content-Type", "application/json")
			w.Write(cf.CreateJsonErrorResponse("Error-Get-login-CF", "invalid authorization header"))
			return
		}
		...
		if user != FakeCfConfig.User || pass != FakeCfConfig.Pass {
			w.WriteHeader(http.StatusUnauthorized)
			w.Header().Set("Content-Type", "application/json")
			w.Write(cf.CreateJsonErrorResponse("Error-Get-login-CF", "invalid login credentials"))
			return
		}
	} else {
		...
		if clientId != FakeCfConfig.ClientId || secret != FakeCfConfig.Secret {
			w.WriteHeader(http.StatusUnauthorized)
			w.Header().Set("Content-Type", "application/json")
			w.Write(cf.CreateJsonErrorResponse("Error-Get-login-CF", "invalid client credentials"))
			return
		}
