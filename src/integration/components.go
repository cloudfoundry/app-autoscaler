...
func (components *Components) PrepareSchedulerConfig(dbUri string, scalingEngineUri string, tmpDir string, consulPort string, httpRequestTimeout time.Duration) string {
...
`
	settingJsonStr := fmt.Sprintf(settingStrTemplate, jdbcDBUri, userName, password, jdbcDBUri, userName, password, scalingEngineUri, testCertDir, testCertDir, testCertDir, testCertDir, components.Ports[Scheduler], components.Ports[Scheduler], consulPort, int(httpRequestTimeout/time.Second))
...
func (components *Components) PrepareMetricsCollectorConfig(dbURI string, port int, ccNOAAUAAURL string, cfGrantTypePassword string, collectInterval time.Duration,
	refreshInterval time.Duration, saveInterval time.Duration, collectMethod string, httpRequestTimeout time.Duration, tmpDir string) string {
	cfg := mcConfig.Config{
		CF: cf.CFConfig{
			API:       ccNOAAUAAURL,
...
func (components *Components) PrepareEventGeneratorConfig(dbUri string, port int, metricsCollectorURL string, scalingEngineURL string, aggregatorExecuteInterval time.Duration,
	policyPollerInterval time.Duration, saveInterval time.Duration, evaluationManagerInterval time.Duration, httpRequestTimeout time.Duration, tmpDir string) string {
...
func (components *Components) PrepareScalingEngineConfig(dbURI string, port int, ccUAAURL string, cfGrantTypePassword string, httpRequestTimeout time.Duration, tmpDir string) string {
...
func (components *Components) PrepareOperatorConfig(dbURI string, ccUAAURL string, cfGrantTypePassword string, scalingEngineURL string, schedulerURL string, syncInterval time.Duration, cutOffDays int, httpRequestTimeout time.Duration, tmpDir string) string {
