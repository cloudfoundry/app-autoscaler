...
`)
			...
			It("returns the config", func() {
				...
				Expect(conf.DataPrune.CutoffDays).To(Equal(20))
			})
		})
...
`)
			...
			It("returns default values", func() {
				...
				Expect(conf.DataPrune.CutoffDays).To(Equal(DefaultCutoffDays))
			})
		})
