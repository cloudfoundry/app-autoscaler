...
describe('API Server-Scheduler Integration tests', function(){
  ...
  after(function(done) {
	...
	context('Policies with schedules',function(){
		...
		it('creates a policy and associated schedules',function(done){
			async.series([	
		...
		it('fails with an invalid policy',function(done){
			var invalidPolicy=JSON.parse(JSON.stringify(fakePolicy));
			invalidPolicy.instance_min_count=5;
			async.series([
        function createDummyPolicy(callback){
          var createOptions={
              url:apiServerURI+'/v1/policies/dummy',
	...
	context('Policies without schedules',function(){
		// Cleanup the DB before running every context
		beforeEach(function(done){
			var deletePolicyOptions={
					url:apiServerURI+'/v1/policies/dummy',
		...
		it('creates only the policy',function(done){
	...
	context('Update policies with schedules',function(){
		before(function(done){
			var deletePolicyOptions={
					url:apiServerURI+'/v1/policies/dummy',
					method:'DELETE',
		...
		it('updates the policy and schedules',function(done){
	...
	context('Delete Policies',function(){
		context('for a non-existing app', function () {
			...
			it('Should return a NOT FOUND (404)',function(done){
        var deletePolicyOptions={
            url:apiServerURI+'/v1/policies/dummy',
            method:'DELETE',
            json:true
		...
		context('with an existing app', function() {
			before(function(done){
				var deletePolicyOptions={
						url:apiServerURI+'/v1/policies/dummy',
						method:'DELETE',
			...
			it('deletes the policy and schedules',function(done){
				async.series([
          function getDummyPolicy(callback){
            var policyOptions={
                url:apiServerURI+'/v1/policies/dummy',
