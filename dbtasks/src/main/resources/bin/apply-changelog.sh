...
# Resolve java binary: explicit JAVA_BIN > JAVA_HOME > buildpack default
JAVA_BIN_FROM_HOME="${JAVA_HOME:+${JAVA_HOME}/bin/java}"
JAVA_BIN="${JAVA_BIN:-${JAVA_BIN_FROM_HOME:-/home/vcap/app/.java-buildpack/open_jdk_jre/bin/java}}"
CERTS_DIR="$(mktemp -d)"
mkdir -p "${CERTS_DIR}"
