...
module.exports = function(metricsCollectorSettings) {
  ...
  metricsCollectorUtilObj.getMetricHistory = function getMetricHistory(parameters, callback) {
    ...
    logger.info("Get metric histories", { "appId": appId, "metricType": metricType, "startTime": startTime, "endTime": endTime, "order": order });
    var options = getOptions(appId, metricType, startTime, endTime, order);
    request(options, function(error, response, body) {
      if (error) {
        logger.error("Error occurred during get metric histories ", { "appId": appId, "metricType": metricType, "startTime": startTime, "endTime": endTime, "order": order, "error": error });
        error.statusCode = HttpStatus.INTERNAL_SERVER_ERROR;
        callback(error, null);
      } else if (response.statusCode === HttpStatus.OK) {
        logger.info("Get metric histories successfully", { "appId": appId, "metricType": metricType, "startTime": startTime, "endTime": endTime, "order": order });
        ...
        logger.error("Error occurred during getting metric histories ", { "appId": appId, "metricType": metricType, "startTime": startTime, "endTime": endTime, "order": order, "error": errorObj });
