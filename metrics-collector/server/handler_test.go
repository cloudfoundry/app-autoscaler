...
var _ = Describe("Handler", func() {
	...
	Describe("NewHandler", func() {
		...
		Context("when cf client is not logged in", func() {
			BeforeEach(func() {
				cfc = fakes.NewCfClient(fakes.FAKE_DOPPLER_ACCESS_TOKEN, fakes.FAKE_DOPPLER_URL)
			})

			It("should return empty token and endpoint", func() {
				Expect(handler.GetDopplerAddr()).To(BeEmpty())
				Expect(handler.GetAccessToken()).To(BeEmpty())
			})
		})

		Context("when cf client is logged in", func() {
			BeforeEach(func() {
				cfc = fakes.NewCfClient(fakes.FAKE_DOPPLER_ACCESS_TOKEN, fakes.FAKE_DOPPLER_URL)
				cfc.Login()
			})
