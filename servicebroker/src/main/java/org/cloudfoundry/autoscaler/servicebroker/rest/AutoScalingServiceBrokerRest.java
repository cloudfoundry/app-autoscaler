...
import org.cloudfoundry.autoscaler.servicebroker.util.LocaleUtil;
...
public class AutoScalingServiceBrokerRest {
    ...
    public Response catalog(@Context final HttpServletRequest httpServletRequest) {
        ...
        return RestApiResponseUtil.getResponse(Response.Status.OK, catalog.toString());
    }
    ...
    public Response provisionService(@Context final HttpServletRequest httpServletRequest, String jsonRequestStr,
        ...
        try {
        	...
        	return RestApiResponseUtil.getResponseError(MESSAGE_KEY.ParseJSONError, e, LocaleUtil.getLocale(httpServletRequest));
        }

    	String msgPrefix = String.format("Provisioning service for serviceId %s , orgId %s, spaceId %s ", serviceId, orgId, spaceId);
        ...
        try {
            ...
            return RestApiResponseUtil.getResponse(Response.Status.CREATED, responseBody);
            
        } catch (Exception e){
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(Response.Status.INTERNAL_SERVER_ERROR, e);
        } 
    ...
    public Response bindService(@Context final HttpServletRequest httpServletRequest, String jsonRequestStr,
    	...
    	String msgPrefix = String.format("Bind service for serviceId %s , bindingId %s, appId %s ", serviceId, bindingId, appId);
        logger.info(Constants.MSG_ENTRY + msgPrefix);
        ...
        try {
        	...
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(MESSAGE_KEY.AlreadyBindedAnotherService, e, httpServletRequest.getLocale());
        } catch (ServerUrlMappingNotFoundException e) {
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(MESSAGE_KEY.ServerUrlMappingNotFound, e, httpServletRequest.getLocale());
        } catch (ScalingServerFailureException e) {
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(MESSAGE_KEY.BindServiceFail, e, httpServletRequest.getLocale());
        } catch (Exception e){
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(Response.Status.INTERNAL_SERVER_ERROR, e);
        } 
    ...
    public Response unbindService(@Context final HttpServletRequest httpServletRequest,
        ...
        try {
        	...
        	return RestApiResponseUtil.getResponse(Response.Status.OK, new JSONObject());
        } catch (ServiceBindingNotFoundException e) {
        	logger.info(Constants.MSG_SUCCESS + msgPrefix + " with GONE");        	
        	return RestApiResponseUtil.getResponse(Response.Status.GONE, new JSONObject());
        } catch (ScalingServerFailureException e) {
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(MESSAGE_KEY.UnbindServiceFail, e, LocaleUtil.getLocale(httpServletRequest));
        } catch (Exception e){
        	logger.error(Constants.MSG_FAIL + msgPrefix + " with execption " + e.getMessage(), e);
        	return RestApiResponseUtil.getResponseError(Response.Status.INTERNAL_SERVER_ERROR, e);
        } 
