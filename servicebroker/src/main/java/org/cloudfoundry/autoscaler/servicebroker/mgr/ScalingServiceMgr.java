...
public class ScalingServiceMgr {
    ...
    public JSONObject bindService(String appId, String serviceId, String bindingId) throws AlreadyBoundAnotherServiceException, ServerUrlMappingNotFoundException, ScalingServerFailureException {
    ...
    public void unbindService( String serviceId, String bindingId) throws ServiceBindingNotFoundException, ScalingServerFailureException {
		...
		try {
			...
			if (ConfigManager.getBoolean("forceRemove", false))
				logger.error("unbind FORCE REMOVE with error on service " + serviceId + " and binding id" + bindingId + " ,error: " + e.getMessage(),e);
			else 
				throw e;
		}
    ...
    private boolean alreadyHasOtherServiceBound(String appId, String serviceId, String bindingId) throws AlreadyBoundAnotherServiceException {
    	
    	List<ApplicationInstance> applications = dataService.getBoundAppByAppId(appId);
		if (applications != null && applications.size() > 0) {
			//if there are binding record for the same application, we need to check whether previous binding happened with the same app and same service, but just the different binding id 
			Iterator<ApplicationInstance> iter = applications.iterator();
			while ( iter.hasNext()){
				ApplicationInstance application = iter.next();
				if (application == null || (application.getServiceId().equalsIgnoreCase(serviceId) && application.getBindingId().equalsIgnoreCase(bindingId))){
					iter.remove();
					continue;
				}
			}
	...
	private void unregisterAppWithServer(String appId, String serviceId, String bindingId, String serverUrl) throws ScalingServerFailureException{

        String unregisterUrl = serverUrl + Constants.UNREGISTER_APP_URL
        									.replace("{bindingId}", bindingId).replace("{serviceId}", serviceId)
        									.replace("{appId}", appId);
        		            
        logger.info("unregisterUrl is  " + unregisterUrl);
        String authorization = ConfigManager.getInternalAuthToken();
        ClientResponse response  = restClient.resource(unregisterUrl)
        		.header("Authorization", "Basic " + authorization)
    			.type(MediaType.APPLICATION_JSON)
        ...
        if (responseCode != 204) {
            String jsonResponse = response.getEntity(String.class);
            logger.error("unbind failed for service " + serviceId + " and bindingid " + bindingId + " with response: " + responseCode + ", reason" + jsonResponse);
            String errorMsg = null;
            try {
            	JSONObject jsonObject = new JSONObject(jsonResponse);
            	errorMsg = (String) jsonObject.get("error");
