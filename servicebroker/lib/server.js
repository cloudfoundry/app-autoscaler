...
module.exports = function(settings, catalog, callback) {
    ...
    var publicServer;
    if(settings.tls){
        publicServer = https.createServer(options, app).listen(publicPort, function() {
            var port = publicServer.address().publicPort;
            logger.info('Service broker app is running in secure mode', { port: publicPort });
        });
    }else{
        publicServer = http.createServer(app).listen(publicPort, function() {
            var port = publicServer.address().publicPort;
            logger.info('Service broker app is running', { port: publicPort });
        });
    }
