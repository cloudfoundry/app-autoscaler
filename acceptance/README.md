...
#### Recommended: Using an Existing Organization (Open Source)

For open source development and testing, it is recommended to run acceptance tests on an existing Cloud Foundry organization. This approach requires:

- **Org Manager** role on the target organization
- **Space Admin** role on the target space (or ability to create spaces)
...
| **existing_user**                                   |  optional   |         ""         | Name of the existing user to use. This user should have **Org Manager** and **Space Admin** roles.                                                                                                                                                                                                                           |
...
| **use_existing_organization**                       |  optional   |       false        | Set to `true` to run tests on an existing organization instead of creating a new one. **Recommended for production-like environments** - requires OrgManager role, preserves org after tests.                                                                                                                               |
| **existing_organization**                           |  optional   |         ""         | Name of the existing organization to use when `use_existing_organization` is true. The test user must have **Org Manager** role on this organization.                                                                                                                                                                        |
| **skip_service_access_management**                  |  optional   |       false        | Set to `true` when service access is already enabled for the org (skips enable/disable operations). **Required in OrgManager mode** since the user lacks admin privileges to manage service access.                                                                                                                          |
| **add_existing_user_to_existing_space**             |  optional   |        true        | When using an existing organization, automatically add the user to spaces created during tests.                                                                                                                                                                                                                              |
| **use_existing_space**                              |  optional   |       false        | Set to `true` to use an existing space instead of creating new ones during tests.                                                                                                                                                                                                                                            |
